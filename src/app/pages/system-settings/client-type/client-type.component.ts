...
export class ClientTypeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.showList();
    this.common.aClickedClientTypeEvent
  ...
  createModel() {
    const dialogRef = this.dialog.open( ClientType, {
      width: '500px',
      data: {
        client_type: '',
  ...
  get_list(postdate) {
    this.dataService.create(postdate, 'client-type-list')
    .subscribe(
      data => {
  ...
  deleteModel(type) {
    const dialogRef = this.dialog.open( DeleteType, {
      data: {
        type_id: type.id,
        client_type: type.client_type,
        api_token: this.tokenId,
...
@Component({
  selector: 'app-client-type-model',
  templateUrl: './client-type-model.html',
  styleUrls: ['./client-type.component.scss']
})
export class ClientType {
  // tslint:disable-next-line: max-line-length
  constructor(public dialogRef: MatDialogRef<ClientType>,
  ...
  onClick(): void {
    this.dataService.create(this.data, 'client-type')
    .subscribe(data => {
      if (data.response === 200) {
        this.dialogRef.close();
        this.common.openTost('success','SUCCESS',data.message);
        this.common.aClickedClientTypeEvent.emit('Component A is clicked!!');
      // tslint:disable-next-line: max-line-length
      } else if (data.response === 400) {
...
@Component({
  selector: 'delete-type-model',
  templateUrl: './delete-type-model.html',
  styleUrls: ['./client-type.component.scss']
})
export class DeleteType {
  // tslint:disable-next-line: max-line-length
  constructor(public dialogRef: MatDialogRef<DeleteType>,
    @Inject(MAT_DIALOG_DATA) public data: DialogData,
    private dataService: DataService,
    private common: CommonService) {
  }
  ...
  onClick(): void {
    this.dataService.create(this.data, 'client-type-delete')
    .subscribe(data => {
      if (data.response === 200) {
        this.dialogRef.close();
        this.common.openTost('success','SUCCESS',data.message);
