...
export class HowItWorkComponent implements OnInit {
  ...
  permission(type) {
    return this.common.permission('ap/how-it-works',type);
  }

  createModel() {
    const dialogRef = this.dialog.open( CreateHIW, {
      width: '900px',
      data: {
        title_en: '',
        title_bn: '',
  ...
  openRedModel(hiw) {
    const dialogRef = this.dialog.open( ReadHIW, {
      width: '900px',
      data: {
        hiw_id: hiw.id,
        title_en: hiw.title_en,
        title_bn: hiw.title_bn,
        description_en: hiw.description_en,
        description_bn: hiw.description_bn,
        old_image:hiw.image,
        api_token: this.tokenId,
        operation: 'Read',
        status: hiw.status == 1?true:false,
      }
    });
    dialogRef.afterClosed().subscribe( result => {
    });
  ...
  get_list(postdate) {
    this.dataService.create(postdate, 'ap-hiw-list')
    .subscribe(
      data => {
  ...
  deleteModel(hiw) {
    const dialogRef = this.dialog.open( DeleteHIW, {
      data: {
        hiw_id: hiw.id,
        title_en: hiw.title_en,
        old_image:hiw.image,
...
export class CreateHIW {
  ...
  constructor(public dialogRef: MatDialogRef<CreateHIW>,
  ...
  onClick(): void {
    ...
    this.dataService.create(this.data, 'ap-hiw-add-update')
    .subscribe(data => {
      if (data.response === 200) {
...
export class ReadHIW {
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  // tslint:disable-next-line: max-line-length
  constructor(public dialogRef: MatDialogRef<ReadHIW>,
    @Inject(MAT_DIALOG_DATA) public data: DialogData,
    private dataService: DataService,
    private common: CommonService) {
  }
...
export class DeleteHIW {
  // tslint:disable-next-line: max-line-length
  constructor(public dialogRef: MatDialogRef<DeleteHIW>,
    @Inject(MAT_DIALOG_DATA) public data: DialogData,
    private dataService: DataService,
    private common: CommonService) {
  }
  ...
  onClick(): void {
    this.dataService.create(this.data, 'ap-hiw-delete')
    .subscribe(data => {
      if (data.response === 200) {
        this.dialogRef.close();
        this.common.openTost('success','SUCCESS',data.message);
