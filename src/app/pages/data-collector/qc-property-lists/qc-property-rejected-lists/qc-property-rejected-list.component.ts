...
@Component({
  selector: 'app-qc-property-rejected-list',
  templateUrl: './qc-property-rejected-list.component.html',
  styleUrls: ['./qc-property-rejected-list.component.scss']
})
export class QcPropertyRejectedListComponent implements OnInit {
  ...
  openRedModel(property) {
    const dialogRef = this.dialog.open( QcRejectedModel, {
      width: '600px',
      data: {
  ...
  opendeleteModel(property) {
    const dialogRef = this.dialog.open( QcRejectedDeleteModel, {
      data: {
        property_id: property.id,
        api_token: this.tokenId,
        operation: 'Delete',
...
@Component({
  selector: 'qc-rejected',
  templateUrl: './qc-rejected-modal.html',
  styleUrls: ['./qc-property-rejected-list.component.scss']
})
export class QcRejectedModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;

  constructor(public dialogRef: MatDialogRef<QcRejectedModel>,
...
@Component({
  selector: 'qc-delete',
  templateUrl: './delete-reject-qc-model.html',
  styleUrls: ['./qc-property-rejected-list.component.scss']
})
export class QcRejectedDeleteModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;

  constructor(public dialogRef: MatDialogRef<QcRejectedDeleteModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService,
              private common: CommonService) {
  }
