...
@Component({
  selector: 'app-qc-sale-property-pending-list',
  templateUrl: './qc-sale-property-pending-list.component.html',
  styleUrls: ['./qc-sale-property-pending-list.component.scss']
})
export class FSQcSalePendingListComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('for-sale-property/list' + postdate)
      .subscribe(async(res) => {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('for-sale-property/details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( QcSalePendingModel, {
      width: '800px',
      data: {
  ...
  opendeleteModel(property) {
    const dialogRef = this.dialog.open( QcSalePendingDeleteModel, {
      data: {
        pid: property.id,
...
export class QcSalePendingModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<QcSalePendingModel>,
  ...
  onClick(status, id: number) {
    ...
    this.dataService.create(inputData, 'for-sale-property/action')
      .subscribe(data => {
...
export class QcSalePendingDeleteModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;

  constructor(public dialogRef: MatDialogRef<QcSalePendingDeleteModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService,
              private common: CommonService) {
  }
  ...
  onClick() {
    ...
    this.dataService.create(inputData, 'for-sale-property/delete')
      .subscribe(data => {
          this.submitButton = false;
          if (data.code === 200) {
            this.dialogRef.close();
