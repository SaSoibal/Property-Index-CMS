...
@Component({
  selector: 'app-qc-sale-property-rejected-list',
  templateUrl: './qc-sale-property-rejected-list.component.html',
  styleUrls: ['./qc-sale-property-rejected-list.component.scss']
})
export class FSQcSaleRejectedListComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('for-sale-property/rejected-list' + postdate)
      .subscribe(async(res) => {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('for-sale-property/rejected-details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( QcSaleRejectedModel, {
      width: '800px',
      data: {
  ...
  opendeleteModel(property) {
    const dialogRef = this.dialog.open( QcSaleRejectedDeleteModel, {
      data: {
        pid: property.id,
...
@Component({
  selector: 'app-qc-sale-pending',
  templateUrl: './qc-rejected-modal.html',
  styleUrls: ['./qc-sale-property-rejected-list.component.scss']
})
export class QcSaleRejectedModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<QcSaleRejectedModel>,
...
@Component({
  selector: 'app-qc-pending-delete',
  templateUrl: './delete-qc-model.html',
  styleUrls: ['./qc-sale-property-rejected-list.component.scss']
})
export class QcSaleRejectedDeleteModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;

  constructor(public dialogRef: MatDialogRef<QcSaleRejectedDeleteModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService,
              private common: CommonService) {
  }
  ...
  onClick() {
    ...
    this.dataService.create(inputData, 'for-sale-property/rejected-delete')
      .subscribe(data => {
