...
@Component({
  selector: 'app-qc-sale-property-deleted-list',
  templateUrl: './qc-sale-property-deleted-list.component.html',
  styleUrls: ['./qc-sale-property-deleted-list.component.scss']
})
export class FSQcSaleDeletedListComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('for-sale-property/deleted-list' + postdate)
      .subscribe(async(res) => {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('for-sale-property/deleted-details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( QcSaleDeleteddModel, {
      width: '800px',
      data: {
...
@Component({
  ...
  templateUrl: './qc-deleted-modal.html',
  styleUrls: ['./qc-sale-property-deleted-list.component.scss']
})
export class QcSaleDeleteddModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<QcSaleDeleteddModel>,
