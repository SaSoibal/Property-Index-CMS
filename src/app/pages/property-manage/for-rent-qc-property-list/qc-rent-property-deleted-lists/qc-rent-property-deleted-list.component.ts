...
@Component({
  selector: 'app-qc-rent-property-deleted-list',
  templateUrl: './qc-rent-property-deleted-list.component.html',
  styleUrls: ['./qc-rent-property-deleted-list.component.scss']
})
export class FSQcRentDeletedListComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('for-rent-property/deleted-list' + postdate)
      .subscribe(async(res) => {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('for-rent-property/deleted-details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( QcRentDetailsModel, {
      width: '800px',
      data: {
...
@Component({
  ...
  templateUrl: './qc-rent-details-modal.html',
  styleUrls: ['./qc-rent-property-deleted-list.component.scss']
})
export class QcRentDetailsModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<QcRentDetailsModel>,
