...
@Component({
  selector: 'app-qc-rent-property-rejected-list',
  templateUrl: './qc-rent-property-rejected-list.component.html',
  styleUrls: ['./qc-rent-property-rejected-list.component.scss']
})
export class FSQcRentRejectedListComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('for-rent-property/rejected-list' + postdate)
      .subscribe(async(res) => {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('for-rent-property/rejected-details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( QcRentRejectedModel, {
      width: '800px',
      data: {
  ...
  opendeleteModel(property) {
    const dialogRef = this.dialog.open( QcRentRejectedDeleteModel, {
      data: {
        pid: property.id,
...
@Component({
  selector: 'app-qc-rent-rejected',
  templateUrl: './qc-rent-rejected-modal.html',
  styleUrls: ['./qc-rent-property-rejected-list.component.scss']
})
export class QcRentRejectedModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<QcRentRejectedModel>,
  ...
  onClick(status, id: number) {
    ...
    this.dataService.create(inputData, 'for-rent-property/rejected-action')
      .subscribe(data => {
...
@Component({
  selector: 'app-qc-rent-rejected-delete',
  templateUrl: './delete-rent-qc-model.html',
  styleUrls: ['./qc-rent-property-rejected-list.component.scss']
})
export class QcRentRejectedDeleteModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;

  constructor(public dialogRef: MatDialogRef<QcRentRejectedDeleteModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService,
              private common: CommonService) {
  }
  ...
  onClick() {
    ...
    this.dataService.create(inputData, 'for-rent-property/rejected-delete')
      .subscribe(data => {
          this.submitButton = false;
          if (data.code === 200) {
            this.dialogRef.close();
