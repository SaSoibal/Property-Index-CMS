...
@Component({
  ...
  templateUrl: './for-rent-approve-listing.component.html',
  styleUrls: ['./for-rent-approve-listing.component.scss']
})
export class FRApproveListingComponent implements OnInit {
  ...
  filterRentType(event) {
    this.searchRentType = event?event:0;
  }
  ...
  clickFilter() {
      ...
      const queryString = "?"
        ...
        + "&searchRentType=" + this.searchRentType
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('approved-list/for-rent' + postdate)
      .subscribe(async(res) => {
          if (res.code === 200) {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('approved-list/rent-details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( ApprovedRentDetailsModel, {
      width: '900px',
      data: {
  ...
  openListingModel(property){
    const dialogRef = this.dialog.open( RentListingModel, {
      width: '400px',
      data: {
        property_id: property.id,
...
export class ApprovedRentDetailsModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<ApprovedRentDetailsModel>,
...
@Component({
  selector: 'app-rent-listing-modal',
  templateUrl: './rent-listing-modal.html',
  styleUrls: ['./for-rent-approve-listing.component.scss']
})
export class RentListingModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<RentListingModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService, public sanitizer: DomSanitizer,
              private common: CommonService) {
  }
  ...
  onClick(){
    this.dataService.create(this.data, 'approved-list/for-rent-view')
      .subscribe(
        data => {
          if (data.code === 200) {
