...
@Component({
  ...
  templateUrl: './for-land-sale-approve-listing.component.html',
  styleUrls: ['./for-land-sale-approve-listing.component.scss']
})
export class FLSApproveListingComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('approved-list/for-land-sale' + postdate)
      .subscribe(async(res) => {
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( ApprovedLandSaleDetailsModel, {
      width: '900px',
      data: {
  ...
  openListingModel(property){
    const dialogRef = this.dialog.open( LandSaleListingModel, {
      width: '400px',
      data: {
        property_id: property.id,
...
export class ApprovedLandSaleDetailsModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<ApprovedLandSaleDetailsModel>,
...
@Component({
  selector: 'app-land-sale-listing-modal',
  templateUrl: './land-sale-listing-modal.html',
  styleUrls: ['./for-land-sale-approve-listing.component.scss']
})
export class LandSaleListingModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<LandSaleListingModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService, 
              private common: CommonService) {
  }
  ...
  onClick(){
    this.dataService.create(this.data, 'approved-list/for-land-sale-view')
      .subscribe(
        data => {
          if (data.code === 200) {
