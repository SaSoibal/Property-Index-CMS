...
@Component({
  ...
  templateUrl: './for-rent-reassign-property.component.html',
  styleUrls: ['./for-rent-reassign-property.component.scss']
})
export class FRReassignPropertyComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('reassign-property/for-rent' + postdate)
      .subscribe(async(res) => {
          if (res.code === 200) {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('reassign-property/rent-details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( RentReassignDetailsModel, {
      width: '900px',
      data: {
  ...
  openListingModel(property){
    const dialogRef = this.dialog.open( RentReassignModel, {
      width: '600px',
      data: {
        property_id: property.id,
        details : property,
...
export class RentReassignDetailsModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<RentReassignDetailsModel>,
...
export class RentReassignModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  team_list = [];
  team_member = [];
  constructor(public dialogRef: MatDialogRef<RentReassignModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService, public sanitizer: DomSanitizer,
              private common: CommonService) {
  }
  ...
  onClick(){
    ...
    this.dataService.create(inputData, 'reassign-property/for-rent-reassign')
      .subscribe(data => {
