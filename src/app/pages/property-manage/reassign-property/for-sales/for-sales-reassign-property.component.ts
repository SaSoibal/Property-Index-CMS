...
@Component({
  ...
  templateUrl: './for-sales-reassign-property.component.html',
  styleUrls: ['./for-sales-reassign-property.component.scss']
})
export class FSReassignPropertyComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('reassign-property/for-sales' + postdate)
      .subscribe(async(res) => {
          if (res.code === 200) {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('reassign-property/sales-details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( ReassignDetailsModel, {
      width: '900px',
      data: {
  ...
  openReassignModel(property){
    const dialogRef = this.dialog.open( SaleReassignModel, {
      width: '600px',
      data: {
        property_id: property.id,
        details : property,
...
export class ReassignDetailsModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<ReassignDetailsModel>,
...
export class SaleReassignModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  team_list = [];
  team_member = [];
  constructor(public dialogRef: MatDialogRef<SaleReassignModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService, public sanitizer: DomSanitizer,
              private common: CommonService) {
  }
  ...
  onClick(){
    ...
    this.dataService.create(inputData, 'reassign-property/for-sale-reassign')
      .subscribe(data => {
