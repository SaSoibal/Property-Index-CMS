...
@Component({
  selector: 'app-qc-land-sale-property-pending-list',
  templateUrl: './qc-land-sale-property-pending-list.component.html',
  styleUrls: ['./qc-land-sale-property-pending-list.component.scss']
})
export class FLSQcLandSalePendingListComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('for-land-sale-property/list' + postdate)
      .subscribe(async(res) => {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('for-land-sale-property/details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( QcLandSalePendingModel, {
      width: '800px',
      data: {
  ...
  opendeleteModel(property) {
    const dialogRef = this.dialog.open( QcLandSalePendingDeleteModel, {
      data: {
        pid: property.id,
...
@Component({
  ...
  templateUrl: './qc-land-pending-modal.html',
  styleUrls: ['./qc-land-sale-property-pending-list.component.scss']
})
export class QcLandSalePendingModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<QcLandSalePendingModel>,
...
export class QcLandSalePendingDeleteModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;

  constructor(public dialogRef: MatDialogRef<QcLandSalePendingDeleteModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService,
              private common: CommonService) {
  }
  ...
  onClick() {
    ...
    this.dataService.create(inputData, 'for-land-sale-property/delete')
      .subscribe(data => {
          this.submitButton = false;
          if (data.code === 200) {
            this.dialogRef.close();
