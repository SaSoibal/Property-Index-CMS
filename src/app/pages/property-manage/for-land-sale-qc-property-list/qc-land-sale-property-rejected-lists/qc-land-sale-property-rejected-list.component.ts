...
@Component({
  selector: 'app-qc-land-sale-property-rejected-list',
  templateUrl: './qc-land-sale-property-rejected-list.component.html',
  styleUrls: ['./qc-land-sale-property-rejected-list.component.scss']
})
export class FLSQcLandSaleRejectedListComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('for-land-sale-property/rejected-list' + postdate)
      .subscribe(async(res) => {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('for-land-sale-property/rejected-details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( QcLandSaleRejectedModel, {
      width: '800px',
      data: {
  ...
  opendeleteModel(property) {
    const dialogRef = this.dialog.open( QcLandSaleRejectedDeleteModel, {
      data: {
        pid: property.id,
...
@Component({
  selector: 'app-qc-land-sale-pending',
  templateUrl: './qc-land-rejected-modal.html',
  styleUrls: ['./qc-land-sale-property-rejected-list.component.scss']
})
export class QcLandSaleRejectedModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<QcLandSaleRejectedModel>,
  ...
  onClick(status, id: number) {
    ...
    this.dataService.create(inputData, 'for-land-sale-property/action')
      .subscribe(data => {
...
@Component({
  selector: 'app-land-qc-pending-delete',
  templateUrl: './delete-land-qc-model.html',
  styleUrls: ['./qc-land-sale-property-rejected-list.component.scss']
})
export class QcLandSaleRejectedDeleteModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;

  constructor(public dialogRef: MatDialogRef<QcLandSaleRejectedDeleteModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService,
              private common: CommonService) {
  }
  ...
  onClick() {
    ...
    this.dataService.create(inputData, 'for-land-sale-property/rejected-delete')
      .subscribe(data => {
          this.submitButton = false;
          if (data.code === 200) {
            this.dialogRef.close();
