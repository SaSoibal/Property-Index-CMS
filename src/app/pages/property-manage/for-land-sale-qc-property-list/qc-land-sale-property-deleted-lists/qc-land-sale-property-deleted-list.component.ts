...
@Component({
  selector: 'app-qc-land-sale-property-deleted-list',
  templateUrl: './qc-land-sale-property-deleted-list.component.html',
  styleUrls: ['./qc-land-sale-property-deleted-list.component.scss']
})
export class FLSQcLandSaleDeletedListComponent implements OnInit {
  ...
  get_list(postdate) {
    ...
    this.dataService.getAll('for-land-sale-property/deleted-list' + postdate)
      .subscribe(async(res) => {
  ...
  openRedModel(property) {
    const queryString = "?"
      ...
      + "&pid=" + property.id;
    this.pageBuffer = true;
    this.dataService.getAll('for-land-sale-property/deleted-details' + queryString)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( QcLandSaleDetailsModel, {
      width: '800px',
      data: {
...
@Component({
  selector: 'app-qc-land-sale-details',
  templateUrl: './qc-land-details-modal.html',
  styleUrls: ['./qc-land-sale-property-deleted-list.component.scss']
})
export class QcLandSaleDetailsModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<QcLandSaleDetailsModel>,
