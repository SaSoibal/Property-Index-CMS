...
export class ProductComponent implements OnInit {
  ...
  get_property_count() {
    ...
    this.dataService.getAll('property-manage/sale-product-count' + getData)
      .subscribe(async(res) => {
          if (res.code === 200) {
  ...
  get_list(postdate) {
    this.pageBuffer = true;
    this.dataService.getAll('property-manage/sale-product-list' + postdate)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( ProductDetailsModel, {
      width: '900px',
      data: {
        property_id: property.property_id,
        details : property?.sale_property,
        thumbnail : property?.sale_thumbnail,
        floorplan : property?.sale_floorplan,
        imagevideo : property?.sale_imagevideo,
  ...
  openStepModel(product) {
    const dialogRef = this.dialog.open( ProductStepModel, {
      width: '600px',
      data: {
...
export class ProductDetailsModel {
  ...
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<ProductDetailsModel>,
...
@Component({
  selector: 'app-product-step-modal',
  templateUrl: './product-step-modal.html',
  styleUrls: ['./product.component.scss'],
})
export class ProductStepModel {
  submitButton = false;
  basicImageFile: any;
  today = new Date();
  tokenId = this.common.mycookie.bearertoken;
  ...
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<ProductStepModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService,
