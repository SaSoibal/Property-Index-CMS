...
export class RentProductComponent implements OnInit {
  ...
  get_property_count() {
    ...
    this.dataService.getAll('property-manage/rent-product-count' + getData)
      .subscribe(async(res) => {
          if (res.code === 200) {
  ...
  get_list(postdate) {
    this.pageBuffer = true;
    this.dataService.getAll('property-manage/rent-product-list' + postdate)
      .subscribe(async(res) => {
          this.pageBuffer = false;
          if (res.code === 200) {
  ...
  detailsModel(property, details) {
    ...
    const dialogRef = this.dialog.open( RentProductDetailsModel, {
      width: '900px',
      data: {
        property_id: property.property_id,
        details : property?.rent_property,
        thumbnail : property?.rent_thumbnail,
        floorplan : property?.rent_floorplan,
        imagevideo : property?.rent_imagevideo,
  ...
  openStepModel(product) {
    const dialogRef = this.dialog.open( RentProductStepModel, {
      width: '600px',
      data: {
...
@Component({
  selector: 'app-rent-product-details-modal',
  templateUrl: './rent-product-details-modal.html',
  styleUrls: ['./rent-product.component.scss'],
})
export class RentProductDetailsModel {
  submitButton = false;
  tokenId = this.common.mycookie.bearertoken;
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<RentProductDetailsModel>,
...
@Component({
  selector: 'app-rent-product-step-modal',
  templateUrl: './rent-product-step-modal.html',
  styleUrls: ['./rent-product.component.scss'],
})
export class RentProductStepModel {
  submitButton = false;
  basicImageFile: any;
  today = new Date();
  tokenId = this.common.mycookie.bearertoken;
  ...
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  constructor(public dialogRef: MatDialogRef<RentProductStepModel>,
              @Inject(MAT_DIALOG_DATA) public data: DialogData,
              private dataService: DataService,
