...
export class AssignLeadComponent implements OnInit {
  ...
  permission(type) {
    return this.common.permission('lead-manage/lead-assign',type);
  }
  ...
  assignModel() {
    ...
    const dialogRef = this.dialog.open( AssignModel, {
      width: '500px',
      data: {
  ...
  get_list(curl) {
    ...
    this.dataService.getAll('lead-assign-list/'+curl)
...
@Component({
  selector: 'assign-model',
  templateUrl: './assign-model.html',
  styleUrls: ['./assign-lead.component.scss']
})
export class AssignModel {
  ...
  constructor(public dialogRef: MatDialogRef<AssignModel>,
  ...
  onClick(): void {
    ...
    this.dataService.create(dataList, 'assign-lead')
    .subscribe(data => {
      if (data.response === 200) {
        this.dialogRef.close();
        this.common.openTost('success','SUCCESS',data.message);
        this.common.onAssignLeadEvent.emit('Component A is clicked!!');
      // tslint:disable-next-line: max-line-length
      } else if (data.response === 400) {
