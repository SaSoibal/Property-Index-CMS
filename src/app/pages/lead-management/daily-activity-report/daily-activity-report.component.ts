...
export class DailyActivityReportComponent implements OnInit {
  ...
  permission(type) {
    return this.common.permission('lead-manage/daily-activity-report',type);
  }
  ...
  get_list(curl) {
    if(this.changeBranch){
      ...
      this.dataService.getAll('get-daily-activity-report/'+curl)
  ...
  clickCountForDetails(leads,step){
    ...
    const dialogRef = this.dialog.open( StepActivityWiseLeadModel, {
      width: '1000px',
      data: {
        step: step,
        others: '',
  ...
  clickOtherCountForDetails(leads,others){
    const dialogRef = this.dialog.open( StepActivityWiseLeadModel, {
      width: '1000px',
      data: {
        step: '',
...
export class StepActivityWiseLeadModel implements OnInit {
  tokenId = this.common.mycookie.bearertoken ;
  pageBuffer = false;
  collection = [];
  totalrow = 0;
  ...
  constructor(public dialogRef: MatDialogRef<StepActivityWiseLeadModel>,
  ...
  get_list() {
    ...
    + "&api_token=" +  this.tokenId;
    this.pageBuffer = true;
    this.dataService.getAll('get-daily-activity-lead-details/'+curl)
    .subscribe(async(data)=> {
        this.pageBuffer = false;
        if (data.response === 200) {
          this.collection = await data.list.data;
