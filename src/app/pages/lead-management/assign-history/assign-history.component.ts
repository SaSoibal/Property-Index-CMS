...
export class AssignHistoryComponent implements OnInit {
  ...
  permission(type) {
    return this.common.permission('lead-manage/assign-history',type);
  }
  ...
  get_list(curl) {
    if(this.changeBranch){
      ...
      this.dataService.getAll('lead-history-list/'+curl)
  ...
  clickCountForDetails(leads,step){
    const dialogRef = this.dialog.open( AssignLeadDetModel, {
      width: '1000px',
      data: {
        step: step,
...
@Component({
  selector: 'history-details-model',
  templateUrl: './history-details-model.html',
  styleUrls: ['./assign-history.component.scss']
})
export class AssignLeadDetModel implements OnInit {
  tokenId = this.common.mycookie.bearertoken ;
  pageBuffer = false;
  collection = [];
  totalrow = 0;
  ...
  constructor(public dialogRef: MatDialogRef<AssignLeadDetModel>,
  ...
  get_list() {
    ...
    + "&api_token=" +  this.tokenId;
    this.pageBuffer = true;
    this.dataService.getAll('lead-history-report/'+curl)
    .subscribe(async(data)=> {
        this.pageBuffer = false;
        if (data.response === 200) {
          this.collection = await data.list.data;
