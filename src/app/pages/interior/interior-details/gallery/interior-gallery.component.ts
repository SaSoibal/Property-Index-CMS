...
@Component({
  selector: 'app-interior-gallery',
  templateUrl: './interior-gallery.component.html',
  styleUrls: ['./interior-gallery.component.scss']
})
export class InteriorGalleryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.showList();
    this.common.aClicKInteriorGallery
  ...
  createModel() {
    const dialogRef = this.dialog.open( CreateGallery, {
      width: '400px',
      data: {
  ...
  get_list(postdate) {
    this.dataService.getAll('gallery-list' + postdate)
      .subscribe(async(res) => {
          if (res.response === 200) {
  ...
  deleteModel(cs) {
    const dialogRef = this.dialog.open( DeleteGallery, {
      data: {
        cs_id: cs.id,
        old_image: cs.image,
        api_token: this.tokenId,
...
export class CreateGallery {
  ...
  constructor(public dialogRef: MatDialogRef<CreateGallery>,
  ...
  onClick(): void {
    ...
    this.dataService.create(this.data, 'gallery-create-update')
    .subscribe(data => {
      if (data.response === 200) {
        this.dialogRef.close();
        this.common.openTost('success', 'SUCCESS', data.message);
        this.common.aClicKInteriorGallery.emit('Component A is clicked!!');
      // tslint:disable-next-line: max-line-length
...
@Component({
  selector: 'delete-gallery-modal',
  templateUrl: './delete-model.html',
  styleUrls: ['./interior-gallery.component.scss']
})
export class DeleteGallery {
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  // tslint:disable-next-line: max-line-length
  constructor(public dialogRef: MatDialogRef<DeleteGallery>,
    @Inject(MAT_DIALOG_DATA) public data: DialogData,
    private dataService: DataService,
    private common: CommonService) {
  }
  ...
  onClick(): void {
    this.dataService.create(this.data, 'gallery-delete')
    .subscribe(data => {
      if (data.response === 200) {
        this.dialogRef.close();
        this.common.openTost('success', 'SUCCESS', data.message);
