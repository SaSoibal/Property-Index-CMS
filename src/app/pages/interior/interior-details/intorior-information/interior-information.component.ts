...
@Component({
  selector: 'app-interior-information',
  templateUrl: './interior-information.component.html',
  styleUrls: ['./interior-information.component.scss']
})
export class InteriorInformationComponent implements OnInit {
  ...
  createModel() {
    const dialogRef = this.dialog.open( CreateInteriorInfo, {
      width: '900px',
      data: {
        title_en: '',
        title_bn: '',
  ...
  openRedModel(cs) {
    const dialogRef = this.dialog.open( ReadInteriorInfo, {
      width: '900px',
      data: {
        cs_id: cs.id,
  ...
  get_list(postdate) {
    this.dataService.getAll('interior-info-list' + postdate)
      .subscribe(async(res) => {
          if (res.response === 200) {
  ...
  deleteModel(cs) {
    const dialogRef = this.dialog.open( DeleteInteriorInfo, {
      data: {
        cs_id: cs.id,
        title_en: cs.title_en,
        old_image: cs.thumbnail,
...
@Component({
  selector: 'create-intrior-model',
  templateUrl: './create-model.html',
  styleUrls: ['./interior-information.component.scss']
})
export class CreateInteriorInfo {
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  ...
  constructor(public dialogRef: MatDialogRef<CreateInteriorInfo>,
  ...
  onClick(): void {
    ...
    this.dataService.create(this.data, 'interior-info-create-update')
    .subscribe(data => {
      if (data.response === 200) {
        this.dialogRef.close();
        this.common.openTost('success', 'SUCCESS', data.message);
        this.common.aClickIntorioInfo.emit('Component A is clicked!!');
      // tslint:disable-next-line: max-line-length
...
@Component({
  selector: 'read-iterior-model',
  templateUrl: './read-model.html',
  styleUrls: ['./interior-information.component.scss']
})
export class ReadInteriorInfo {
  rootUrl =  this.common.rootUrl + 'public/uploads/';
  // tslint:disable-next-line: max-line-length
  constructor(public dialogRef: MatDialogRef<ReadInteriorInfo>,
    @Inject(MAT_DIALOG_DATA) public data: DialogData,
    private dataService: DataService,
    private common: CommonService) {
  }
...
@Component({
  selector: 'delete-interior-modal',
  templateUrl: './delete-model.html',
  styleUrls: ['./interior-information.component.scss']
})
export class DeleteInteriorInfo {
  // tslint:disable-next-line: max-line-length
  constructor(public dialogRef: MatDialogRef<DeleteInteriorInfo>,
    @Inject(MAT_DIALOG_DATA) public data: DialogData,
    private dataService: DataService,
    private common: CommonService) {
  }
  ...
  onClick(): void {
    this.dataService.create(this.data, 'interior-info-delete')
    .subscribe(data => {
      if (data.response === 200) {
        this.dialogRef.close();
        this.common.openTost('success', 'SUCCESS', data.message);
